...
MODULE_NAME = "dmarc_scan"

def perform_dmarc(domain: str) -> Dict[str, Any]:
...
def build_result(target: str) -> Dict[str, Any]:
    ...
    try:
        result["data"] = perform_dmarc(target)
    except Exception as exc:
        logging.error("DMARC check failed: %s", exc)
        result["status"] = "error"
...
def main(target: Optional[str] = None, json_out: Optional[str] = None, api: Optional[bool] = False) -> int:
    if target is None:
        parser = argparse.ArgumentParser(description="Check DMARC configuration")
