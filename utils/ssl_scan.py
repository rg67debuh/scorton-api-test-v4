...
def perform_scan(target: str) -> Dict[str, Any]:
    cmd = ["sslyze", "--regular", "--json_out=-", target]
...
def build_result(target: str) -> Dict[str, Any]:
    ...
    try:
        result["data"] = perform_scan(target)
    except Exception as exc:
        logging.error("SSL scan failed: %s", exc)
        result["status"] = "error"
...
def main(target: Optional[str] = None, json_out: Optional[str] = None, api: Optional[bool] = False) -> int:
    if target is None:
        parser = argparse.ArgumentParser(description="Scan SSL configuration")
